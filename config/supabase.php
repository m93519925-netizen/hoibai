...
function storage_upload(string $tmp_path, string $original_name, string $folder = 'questions'): ?string {
    ...
    $status = curl_getinfo(($r = curl_exec($ch)) ? $ch : $ch, CURLINFO_HTTP_CODE);
    curl_close($ch);

    return $status === 200
        ? SUPABASE_URL . '/storage/v1/object/public/qa-images/' . $filename
        : null;
}
...
function nl2br_safe(string $str): string {
    return nl2br(h($str));
}
...
function deduct_points(string $user_id, int $amount, string $reason, ?string $ref_id = null): bool {
    ...
    db_update('profiles', "id=eq.{$user_id}", ['points' => $profile['points'] - $amount]);
